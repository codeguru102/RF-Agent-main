...
def parse_args():
    ...
    parser.add_argument("--task-dir", default="tasks/matlab_task")
...
def build_context(args):
    ...
    llm_client = LLMClient(
        ...
        dry_run_reward_signature=task_config.get("reward_signature", "function reward = reward_fcn(state, u_action, prev_u_action, flag)"),
    )
...
def export_best_reward(task_dir: Path, tree: SearchTree):
    ...
    reward_path = task_dir / "best_reward_fcn.m"
...
def sync_candidate_summaries(store: CandidateStore, log_reader: MatlabLogReader):
    synced = []
...
def run_cli() -> int:
    ...
    try:
        ...
        print_error(f"RF-Agent-MATLAB failed: {exc}")
        if args.debug:
            traceback.print_exc()
        else:
    ...
    print_success("RF-Agent-MATLAB finished successfully.")
    return 0
